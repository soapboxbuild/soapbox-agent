...
primary one? These will be reported but won't drive the compliance math."
...
Store as: `primary_target` — `{type: crrem|percent|bps-fine-avoidance|net-zero-year, detail}`
(exactly one), and `secondary_targets` — list of `{type, detail}` (may be empty).
...
`gaps` — list of `{type, provider, expected_date}`.
